import { randomBytes } from "crypto";
interface SecretData {
  secret_data?: string;
  coefficients: Array<Array<number>>;
}
enum ShamirError {
  /// The number of shares must be between 1 and 255
  InvalidShareCount,
}
//@ts-ignore
Array.prototype.insert = function (index, ...items) {
  this.splice(index, 0, ...items);
};
const GF256_LOG = [
  ,
];
const GF256_EXP = [
  ,
];
function with_secret(secret: string, threshold: number): SecretData {
  let coefficients: Array<Array<number>> = [];
  // let rng=[] //figure this out
  let rand_container: Array<number> = new Array(threshold - 1).fill(0);
  let utf8Encode = new TextEncoder();
  // console.log(utf8Encode.encode(secret).values())
  for (let c of Array.from(utf8Encode.encode(secret).values())) {
    // console.log(c)
    //@ts-ignore this is fine
    rand_container = randomBytes(threshold - 1);
    let coef: Array<number> = [c];
    for (let r of rand_container) {
      coef.push(r);
    }
    coefficients.push(coef);
  }
  let sd: SecretData = {
    secret_data: secret,
    coefficients,
  };
  return sd;
}

function accumulate_share_bytes(
  id: number,
  coefficient_bytes: Array<number>
): number | ShamirError {
  if (id == 0) {
    return ShamirError.InvalidShareCount;
  }
  let accumulator: number = 0;

  let x_i: number = 1;

  for (let c of coefficient_bytes) {
    accumulator = gf256_add(accumulator, gf256_mul(c, x_i));
    x_i = gf256_mul(x_i, id);
  }

  return accumulator;
}
function gf256_add(a: number, b: number): number {
  return a ^ b;
}
function gf256_mul(a: number, b: number): number {
  if (a == 0 || b == 0) {
    return 0;
  } else {
    let k = GF256_LOG[a];
    let t = GF256_LOG[b];
    return GF256_EXP[(k + t) % 255];
  }
}
function get_share(self: SecretData, id: number): Array<number> | ShamirError {
  if (id == 0) {
    return ShamirError.InvalidShareCount;
  }
  let share_bytes: Array<number> = [];
  let coefficients = self.coefficients.slice();
  for (let coefficient of coefficients) {
    let b = accumulate_share_bytes(id, coefficient);
    share_bytes.push(b);
  }
  //@ts-ignore
  share_bytes.insert(0, id);
  return share_bytes;
}

// export all functions
export {
  with_secret,
  get_share,
  ShamirError,
  SecretData,
  accumulate_share_bytes,
  gf256_add,
  gf256_mul,
};
